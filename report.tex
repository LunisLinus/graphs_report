...
\DefineVerbatimEnvironment{plaincode}{Verbatim}{fontsize=\footnotesize, breaklines=true, breakanywhere=true, xleftmargin=2em}
...
private void ShowNonAdjacent()
{
    ...
    Для проверки использовался файл \path{/Users/lunis/Documents/graph1.txt}.
...
private void ShowIsolated()
{
    ...
    \textbf{Пример входных данных.}
    Для проверки использовался файл \path{/Users/lunis/Documents/graph2.txt}.
...
private void RemoveEdgesToPendant()
{
    ...
    \textbf{Пример входных данных.}
    Для проверки использовался файл \path{/Users/lunis/Documents/graph4.txt}.
...
private void DFSCollectDepths(T current, int depth, HashSet<T> visited, Dictionary<T, int> depths)
{
    ...
    \textbf{Пример входных данных.}
    Для проверки использовался файл \path{/Users/lunis/Documents/graph6.txt}.
...
while (numTrees > 1)
{
    ...
    \textbf{Пример входных данных.}
    Для проверки использовался файл \path{/Users/lunis/Documents/graph3.txt}.
...
for (int i = 0; i < maxIterations; i++)
{
    ...
    \textbf{Пример входных данных.}
    Для проверки использовался файл \path{/Users/lunis/Documents/graph8.txt}.
...
if (seenCycles.Add(hash))
    ...
    \textbf{Пример входных данных.}
    Для проверки использовался файл \path{/Users/lunis/Documents/graph7.txt}.
...
foreach (var u in _adjacencyList)
{
    foreach (var v in u.Value)
    {
        ...
        if (capacity < 0)
            throw new InvalidOperationException(
                "Алгоритм Эдмондса-Карпа не поддерживает отрицательные пропускные способности.");
...
return maxFlow;
    ...
    \textbf{Пример входных данных.}
    Для проверки использовался файл \path{/Users/lunis/Documents/graph9.txt}.
    Граф соответствует примеру со схемы: источник \smalltt{S}, сток \smalltt{T}, пропускные способности подписаны на дугах.
